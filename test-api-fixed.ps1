...
function Invoke-APITest {
    ...
    try {
        ...
        if ($response.data) {
            Write-Host "   Data received: $($response.data | ConvertTo-Json -Compress)" -ForegroundColor Yellow
        }
...
# 1. Health Check
Write-Host "`n📋 HEALTH & INFO ENDPOINTS" -ForegroundColor Magenta
Invoke-APITest -Uri "/health" -Description "Health Check"
Invoke-APITest -Uri "/" -Description "API Info"

# 2. User Registration
Write-Host "`n👤 USER AUTHENTICATION FLOW" -ForegroundColor Magenta
...
Write-Host "`n🔐 PROTECTED AUTHENTICATION ENDPOINTS" -ForegroundColor Magenta
...
# 5. Wallet Operations
Write-Host "`n💰 WALLET OPERATIONS" -ForegroundColor Magenta
...
# 6. Exchange Rates (with properly escaped URLs)
Write-Host "`n💱 EXCHANGE RATES" -ForegroundColor Magenta
